...
- Use `wrangler pages deploy ...`, not bare `wrangler deploy`. Bare `deploy` targets Workers services and can fail with Cloudflare API auth/service errors even when the Pages token is valid. For `hermes-pages`, the correct deploy target is always `pages deploy <dir> --project-name hermes-pages`.
...
- **Company / capstone research pages** — public engineering-note pages that compare local companies and suggest student-scale project ideas. Use `references/company-capstone-research-pages.md` for the page structure, safe capstone framing, and verification checklist.
- **Schedule/reference pages** — public utility pages with searchable/filterable data tables, e.g. sports schedules. For event schedules with timezone conversion, use `references/sports-schedule-static-pages.md`: fetch authoritative/public API data, convert times with Python `zoneinfo`, label Pacific summer events as PT/PDT correctly, verify expected row counts before and after deploy. For World Cup-style sports pages, keep controls phase-appropriate: group-stage schedule views can use a simplified `Filters` header with Team/Group/Venue/status controls, but remove redundant controls when Gordon says they are no longer useful (e.g. drop Group once group filtering is stale, and drop Venue/status when replacing the list with a knockout bracket). Once the tournament has moved past a knockout round, default the knockout bracket tab to the active/current round (e.g. `R16`, `QF`, `SF`) by changing both the active tab class and the JS `activeBracketStage`, not just the visible label. For shorthand requests like “Default to SF,” follow `references/world-cup-bracket-default-round.md`. Top schedule summary cards should be phase-appropriate too: when asked for “today and tomorrow’s games,” update the heading/loading/empty copy and filter by both local date labels while preserving the canonical hidden schedule cards as the source. If the UI label is “Next game,” do not stop at today/tomorrow: filter out already-started/past events for today/tomorrow, then fall forward to the earliest future event in `scheduleData` and render that whole event date as the next-game section. Verify with a fixed future/past clock via jsdom when the real current date cannot reproduce the edge case. For mobile knockout viewing, prefer a tabbed bracket (`R32`, `R16`, `QF`, `SF`, `3rd`, `Final`) with card-based matchups and progression labels instead of the full bottom schedule list; details are in `references/sports-schedule-static-pages.md`. If Gordon asks for a results/schedule date filter, use exactly `Schedule`, `Results`, and `Results & schedule`, default it to `Schedule`, call `apply()` during initialization so completed games are hidden on first load, and reset back to `Schedule`; implementation/verification notes are in `references/world-cup-filter-update-verification.md`. When updating a sports schedule after games have started, check the relevant public scoreboard API for completed events and add concise final-score badges/labels only for events whose status is completed; verify both expected row count and score-label count after deploy. For completed sports games, prefer compact cards showing only matchup, group/stage, and final score unless the user asks to retain logistics; remove or hide kickoff time, venue/location, broadcaster, and betting/market lines for completed games. If a top “today’s games” card mirrors schedule cards, render/copy from the canonical schedule card so it stays consistent with results and detail changes. **When correcting knockout matchup labels, update every source feeding the visible card:** the visible schedule article text/data-search, embedded `#schedule-data` JSON `match` fields, helper maps/override labels, and any clone/render helper that can rewrite row team names. Do not relabel score rows by array index from `matchLabel` if the authoritative ESPN/API score rows are in home/away order; that caused “Egypt vs Argentina” to render with Egypt assigned Argentina’s score. Keep score row team names from the authoritative result rows and only update link text / aria-label. For post-semifinal placeholder resolution, map both completed winners and completed losers through outgoing refs: finals need `Semifinal N Winner`, while 3rd-place matches need `Semifinal N Loser`; a winner-only helper leaves the next-game card stuck on placeholders or falsely empty. If building an auto-updater, make match-card rewrites resilient to both upcoming-card layout (`date` before `match`) and completed-card layout (`match` first): match the whole `<article class="game-card...">`, then find the ESPN game link inside the body rather than assuming the link is the first child. Otherwise newly completed games can be skipped while already-completed cards still update. If removing a visible count/status note or filter/control section also removes an element used by JS (e.g. `id="count"`, `id="team"`, or `id="reset"`), patch the script defensively (e.g. `if (count) ...`, `teamFilter?.addEventListener(...)`, `teamFilter?.value || ""`) so schedule/bracket rendering still works after the DOM element is gone. If browser automation is unavailable, verify inline JS with `node --check` and verify live source using Python `urllib.request` with a `User-Agent`; text extractors may omit form controls/options. For pages with inline JSON plus a later executable `<script>`, extract only executable scripts for `node --check` (exclude `<script type="application/json">`). When DOM behavior matters and Chrome is unavailable, use a temporary jsdom harness; if `npx -p jsdom node script.js` cannot resolve the package from `/tmp`, create a temp npm project, `npm install jsdom`, then run the probe there.
...
For phone-friendly World Cup knockout brackets, follow `references/world-cup-knockout-bracket-mobile.md`: use round tabs plus stacked cards instead of a full tree, hide but preserve the canonical schedule cards if JS depends on them, only fill locked direct slots, and never naively substitute a selected third-place team into every compatible placeholder. Third-place slots require an authoritative exact assignment (e.g. a `knownThirdPlaceSlots` map keyed by ESPN game ID) before replacing the placeholder. If adding market lines (Kalshi) to knockout games, add them to the hidden canonical schedule cards *and* render them into visible `.bracket-card`s (e.g. `bracketKalshiHtml(ev)` reads the schedule card’s `.kalshi`). Style bracket Kalshi text with the same high-contrast gold used by schedule `.kalshi` (`#fce7a8`), not muted white/gray, so prices are visually distinct. Once R32 placeholders resolve to real teams, update any updater script’s placeholder-to-match override map as well as page JS `knownThirdPlaceSlots`; otherwise the hourly updater may leave stale “not posted yet” Kalshi lines because the source schedule rows still contain FIFA placeholders.

When team-filtering World Cup knockout games, do not only replace the selected team's slot. If another direct slot in the same displayed matchup is already locked (e.g. `Group A 2nd Place` is known while filtering for Canada), replace that slot too. Preferred pattern: add a helper like `knownDirectSlotTeam(groupName, status)` that uses `scenarioStatusesForTeam(...)` and returns a team only when its status set is exactly one locked direct status (`1st in group` or `2nd in group`), then run `replaceKnownDirectSlots(label)` before displaying the filtered knockout label. Verify with a concrete completed group case such as Canada showing `South Africa vs Canada`, not `Group A 2nd Place vs Canada`. The Bracket Focus team filter should hide non-matching `.bracket-card`s, not merely highlight the matching card among all round cards; implement by filtering `knockoutStageEvents(activeBracketStage)` through `bracketMatchLabel(ev, selectedTeam)` and showing a clear “No <team> bracket games found in this round” empty state. Verify with concrete selections (e.g. Canada, United States, Brazil each reduce R32 to one card when those slots are known).

For “Next <team> game” cards after the group stage, do not search only for literal team names in `ev.match`; knockout rows often still contain placeholders (`Group D Winner`, `Group D 2nd Place`, third-place pool labels). Reuse the bracket-label path (`bracketMatchLabel(ev, selectedTeam)` / known direct slot helpers / exact `knownThirdPlaceSlots`) to detect whether a locked placeholder resolves to the team, and render the resolved label in the card. Verify with the live scenario that USA’s next game resolves from `Group D Winner vs Third Place Group B/E/F/I/J` to `USA vs Bosnia-Herzegovina` instead of showing “No upcoming USA games found.” If browser automation is unavailable, a lightweight `jsdom` probe can verify the rendered filter behavior by loading the HTML, dispatching `change` on `#team`, and counting `.bracket-card`s; keep the probe temporary unless it becomes a recurring script.
...
For Sierra backcountry fishing candidate pages, also follow `references/backcountry-fishing-candidate-pages.md`: compare against the North Fork San Joaquin / Sheep Crossing benchmark, include no-traffic drive time from Dublin when relevant, distinguish timberline terrain from fishable creek water, link the candidate from the main Backcountry Fishing page, and avoid overselling lake/reservoir scenery as plunge-pool creek fishing.

For backcountry-fishing candidate pages specifically, include logistics viability early, not just scenic itinerary content. Gordon is starting from Dublin, CA; if a candidate is framed as a practical alternative, estimate drive time from Dublin (load/use the `maps` skill or OSRM/Nominatim routing) and classify whether it is sub-4-hour, borderline, or long-haul. If a page is created from an item already listed on `/wiki/hobbies/backcountry-fishing`, update that list item to link to the new page and note any newly discovered viability signal (e.g. “best sub-4-hour candidate from Dublin”).

For backcountry fishing destination/detail pages, also follow `references/backcountry-fishing-itinerary-pages.md`: evaluate fit against Gordon's NFSJ/Sheep Crossing benchmark, include a concrete 2–3 night itinerary, use authoritative USFS/CDFW sources for logistics/species claims, use local Commons images plus a clearly labeled conceptual route SVG when useful, update the Backcountry Fishing list item to link to the new page, and verify page/index/backlink/assets after Wrangler deploy.

For Gordon's backcountry fishing page specifically, use `references/backcountry-fishing-candidate-research.md` when adding or evaluating Sierra destinations. It captures his Sheep Crossing/NFSJ benchmark, the timberline/alpine variant framing, and candidate notes/sources so future edits don't overstate lake-heavy high-country basins as direct plunge-pool canyon replacements.

For enriching travel itinerary pages, also follow `references/travel-itinerary-page-enrichment.md`: use locally saved Commons/public images, hero sections, mood-board grids, day cards/timelines, playful prompts, and route SVGs; keep itinerary pages public by default, remove existing wiki auth guards unless Gordon requested privacy, handle Wikimedia `Special:FilePath` downloads with exact filenames, URL encoding, retry/backoff for 429s, and live asset verification. When Gordon asks for a quick itinerary addition to an existing page, patch both the visible day content and any inline JS/data arrays that render dynamic “today’s itinerary” panels (e.g. `scotlandDailyPlans`), then grep the file for the added/stale phrase before committing. If the change affects a related day-planning page (e.g. a transfer subpage that points to the next day), patch that related page too so it does not preserve stale guidance. When adding attraction times, verify current official opening-hours pages first, then cite/use those hours in the itinerary. When Gordon uploads an itinerary PDF and the target page is clear, extract it locally, patch the existing HTML in place, redact booking/reference/confirmation IDs, verify no sensitive IDs or labels remain, then commit, Wrangler deploy, and verify the canonical URL.

**Itinerary factual corrections:** when Gordon points out a venue/day mismatch (e.g. market closed Mondays), verify current official/opening-hours sources before patching, then update every representation of that day — visible headings/body bullets/tags **and** any inline JS/data arrays used for prompts or dynamic itinerary cards. Grep the page for the stale venue/phrase before committing so old guidance does not survive in hidden data. For London market substitutions, Borough Market is normally closed Mondays; Seven Dials Market and Old Spitalfields Market are Monday-friendly alternatives, but re-check current hours before relying on them.

### Home renovation wiki pages

For renovation research pages (solar, fixtures, flooring, fixtures, rough-in planning, product research), follow `references/home-renovation-wiki-pages.md`: edit hand-written HTML under `/opt/data/hermes-pages`, place discoverability links in the **Home Renovation** section of `wiki/index.html`, use local SVG/assets where useful, preserve auth/public behavior intentionally, deploy via Wrangler Direct Upload, and verify canonical `hermes-pages-d55.pages.dev` URLs. Keep contractor-facing pages terse and non-redundant: when Gordon calls wording redundant/AI slop, delete the redundant section/bullets rather than preserving them as rephrased checklist items. If the source is a Sidekick Studio share URL, fetch the JSON message payload directly from `/share/<token>/messages` (ignore the `#msg-N` fragment for HTTP; use it only to identify the relevant message), summarize the discussion into durable guidance, and save any useful embedded `data:image/...;base64,...` images as local assets under `/wiki/assets/<topic>/`. See `references/home-renovation-wiki-pages.md` for the exact pattern.

When Gordon asks to add an uploaded bid/spec PDF or other document to a renovation page, treat it as a static asset plus a terse link section: copy the upload from `/opt/data/cache/documents/` into a stable path such as `/opt/data/hermes-pages/wiki/assets/<topic>/<descriptive-name>.pdf`, add a simple section/link on the relevant page (for bathroom docs, `/wiki/projects/bathrooms.html`), commit only the page and asset, deploy via Wrangler, and verify both the page text/link and the PDF URL. If the working tree has unrelated dirty files, use the isolated clone deploy pattern so unrelated work is not published accidentally.
...
- **Git credential file may be empty/stale** — `/opt/data/.git-credentials` is not guaranteed to contain a usable PAT. If push fails with invalid credentials after the intended files are committed locally, do not block the live page on GitHub backup: deploy with Wrangler, verify the canonical URL, and note the repo is locally ahead if that matters.
