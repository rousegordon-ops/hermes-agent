...
- When he gives live-trip corrections or additions (e.g. “we need to check in first,” “George Square is closed,” “the drive is 3 hours, not 1.5,” “our boat trip is only 90 min,” “we already did lunch there,” attraction opening times, or friend recommendations), patch **both** the visible day card/timeline and any `Today's itinerary` JS data object. Also check related companion/transfer pages and planning-link sections for stale versions of the same advice. Grep for stale place names/times/labels across the whole file before committing; hidden JS data can keep wrong guidance alive even after visible HTML is fixed. If a correction changes the day’s premise (e.g. shorter boat trip, already-done stops, a museum opening time, or “do this by car on the way / later by public transit”), update companion pages and planning-link sections too so stale “standard trip,” repeated lunch spots, or already-done attractions do not survive elsewhere.
- When Gordon asks to soften or reframe a specific travel-page line, do not stop at replacing the exact quoted sentence. Search the page for adjacent wording with the same framing (e.g. quick strategy cards, summaries, “not the summit,” “short walk,” or “do only…” language) and update those too so the page tone is consistent.
...
- If the Pages preview URL has the new text but the canonical `hermes-pages-d55.pages.dev` URL is stale immediately after Direct Upload, retry the canonical URL with a browser-ish User-Agent and `Cache-Control: no-cache` for a few short attempts before treating it as a failed deploy. In July 2026 itinerary edits, the preview updated first and the canonical URL caught up seconds later.
