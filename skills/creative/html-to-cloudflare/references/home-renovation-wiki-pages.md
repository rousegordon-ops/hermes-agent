# Home renovation wiki pages on hermes-pages

Use this reference when Gordon asks to add or update house-renovation research in the personal wiki (solar, fixtures, electrical rough-in, bathroom products, etc.).

## Placement and URL conventions

- Source repo: `/opt/data/hermes-pages`.
- Wiki hub index: `/opt/data/hermes-pages/wiki/index.html`.
- Renovation research pages often live as HTML files under `wiki/projects/` even when the hub link appears under the **Home Renovation** section.
- Link from the Home Renovation section using extensionless hrefs, e.g. `/wiki/projects/solar-options`.
- Product utility list pages can live at root-level public paths, e.g. `/showerheads`, `/bathroom-vanity-lights`, `/recessed-gimbal-lights`.

## Auth/public handling

- **Default public:** pages Gordon asks you to create or update should be publicly accessible unless he specifically requests private/auth-gated access.
- Do not add the client-side `wiki_auth` login redirect script by default, even for files under `/wiki/projects/...`.
- If an existing page has the auth script and Gordon asks to publish/share/make it public, remove the client-side `wiki_auth` login redirect script and verify both locally and live that `wiki_auth`/`/wiki/login` are absent.
- Only add or preserve auth when Gordon explicitly asks for a private/protected page.
- Root-level utility/product pages should remain public unless Gordon explicitly requests a proper root-scoped auth flow.

## Home-renovation page workflow

1. Read the current target page and `wiki/index.html` before editing.
2. If Gordon provides a Sidekick Studio share URL, fetch the share JSON directly rather than relying on browser rendering:
   - Page HTML fetch may work, but the useful data is usually at `https://sidekickstudio.net/<app>/share/<token>/messages`.
   - The `#msg-N` fragment is not sent over HTTP; use it only to identify the message index in the returned `messages` array.
   - Message `content` may itself be JSON with `text`, `images`, and `pdfs`. Parse it before summarizing.
   - Embedded `data:image/...;base64,...` images can be decoded and saved as local assets under `/opt/data/hermes-pages/wiki/assets/<topic>/`; use descriptive filenames and verify they load live.
3. Create or patch hand-written HTML directly under `/opt/data/hermes-pages/wiki/...`.
   - For renovation pages intended to be shared with contractors/installers, write the entire page in respectful peer-to-peer contractor-facing language. Do not add sections titled “Installer-facing wording”, “Installer-facing summary”, or similar — the whole page is already installer-facing. Frame items as preferences, working assumptions, and discussion points to review with the contractor, not as orders or implications that the contractor needs correction.
   - **Terse edit rule:** when Gordon flags a passage as redundant, unnecessary, or “AI slop,” delete the redundant bullets/section outright unless he explicitly asks for a rewrite. Do not keep the same checklist content under softer wording. If one useful sentence remains, move it into the nearest existing section and remove the extra heading/list. For structural/engineering notes, avoid boilerplate like “engineer to confirm beam material, depth, bearing, connectors, permit needs” when the page already says an engineer/contractor must size the work; keep only the real decision constraint or preference (e.g. PT if it stays under the height limit, LVL fallback) if it materially helps the contractor conversation.
4. Add a simple `<li><a href="...">Title</a></li>` in the appropriate `wiki/index.html` section. No summaries/descriptions in the hub list.
   - If Gordon asks to consolidate several related renovation pages into one shareable page, create a public umbrella page under `wiki/projects/` (e.g. `/wiki/projects/bathrooms`) with concise decision summaries, representative images, contractor/discussion points, and links back to the detailed subpages. Update the Home Renovation hub to link to the umbrella page instead of the consolidated individual links, but leave the detailed pages live and linked from inside the umbrella page.
5. If the page needs illustrations or supporting documents, prefer local self-contained assets under `/opt/data/hermes-pages/wiki/assets/` unless a licensed/downloaded image is necessary.
   - For contractor/reference PDFs supplied by Gordon (bids, portfolios, spec packets), copy the PDF into a topic-specific asset directory such as `/opt/data/hermes-pages/wiki/assets/bathrooms/`, give it a short stable filename, link it from the relevant page with a clear button label, commit the PDF plus page change, deploy, and verify the live asset returns `application/pdf` and starts with `%PDF`.
   - When Gordon sends an annotated floor-plan/image, save a local copy under a topic-specific asset directory, run vision analysis on the image, and add it to the page as a figure with a concise caption.
   - Carry forward plan markup explicitly: measurements, circled dimensions, fixture letters, and rough-in labels. For shower plans, preserve common markup semantics such as `S` = showerhead and `V` = volume/control valve when Gordon identifies them.
   - Keep uncertainty language only where the markup is genuinely unreadable; if Gordon states what a mark means, treat that as authoritative.
   - For product images from retail links, fetch/store local copies under the relevant asset directory and cite the product context in captions. If the retailer blocks direct scraping (Home Depot often returns 403; Amazon often returns empty extraction), use the manufacturer's matching product/gallery pages, ASIN/model-number web search results, reseller mirrors, or indexed product-image URLs as a fallback, while preserving the user's intended retail product name/model/link in the page text. For Amazon ASINs, search the ASIN in quotes, use mirror pages only to identify the title/specs/image, download a stable `m.media-amazon.com` or mirror image locally, and visually verify the saved image before publishing.
   - When Gordon says a product was selected, make the page hierarchy reflect that decision: add a visible “Selected” callout/card near the top, include a product image when available, mark the corresponding product entry as selected, and list/render selected items first on utility-list pages.
   - For conceptual exterior/interior mockups, do not present crude paint-over composites as representative. If a quick rough mockup is still useful, label it explicitly as rough and verify it visually before sending; otherwise prefer adding the reference images and text direction to the page, or use a proper perspective-aware design/rendering workflow.
   - If an existing asset directory or file is root-owned and not writable, do not get stuck trying chmod/sudo. If the parent directory is writable, create a user-owned replacement copy and move the root-owned directory aside; if the archive remains inside the deploy tree, add it to `.git/info/exclude` and deploy from a clean staging copy that excludes it so stale/archive files are not uploaded.
6. Commit only relevant files from `/opt/data/hermes-pages`, push with `GIT_TERMINAL_PROMPT=0`, then deploy via Wrangler Direct Upload:
   ```bash
   npx -y -p node@22 -p wrangler wrangler pages deploy /opt/data/hermes-pages --project-name hermes-pages --commit-dirty=true
   ```
6. Verify canonical URLs on `https://hermes-pages-d55.pages.dev`, not only the Wrangler preview URL.

## Verification pattern

Use Python `urllib.request` with expected text needles for every changed page and asset, e.g.:

```python
import urllib.request
base='https://hermes-pages-d55.pages.dev'
checks={
  '/wiki/projects/solar-options-planning':['Planning for Solar Later','solar-ready-rough-in.svg'],
  '/wiki/projects/solar-options':['Planning while walls are open','/wiki/projects/solar-options-planning'],
}
for path, needles in checks.items():
    html=urllib.request.urlopen(urllib.request.Request(base+path,headers={'User-Agent':'Mozilla/5.0'}),timeout=30).read().decode('utf-8','replace')
    assert all(n in html for n in needles), path
```

## Solar / planning-during-construction notes

For Ventura solar planning pages, Gordon's situation includes a large south-facing roof and open walls during electrical rough-in. The solar planning material should live on a dedicated construction-phase subpage (currently `/wiki/projects/solar-options-planning-during-construction`) rather than being spread across every solar subpage; overview/conventional/decision pages should carry short callouts that link to it. If renaming/moving a wiki page, add a `_redirects` entry from the old extensionless URL and verify the redirect.

Gordon's current preference for this class of work: assume eventual **whole-home backup with smart load management**, but do only construction-phase work that would be significantly more expensive, ugly, or disruptive after drywall. Do not push premature vendor-specific battery/inverter/smart-panel decisions.

Key guidance to preserve in future edits:

- Oversized empty conduit from attic/roof area to electrical equipment area.
- Preserve main-panel/service capacity and spare breaker spaces for solar, battery, EV, and electrification.
- Reserve wall space near main service equipment for future gateway/transfer equipment, meters, disconnects, and load-management hardware.
- If there is no good battery location near the main panel, identify plausible remote battery locations and preserve hidden conduit/pathways from the main electrical/gateway area to those locations; add pull strings and labels, but do not assume final AC vs DC/proprietary conductor architecture.
- Add backing/blocking, working clearance, and serviceable space at likely remote battery/equipment walls where low-cost during construction.
- For likely whole-home backup with smart load management, emphasize gateway/load-management space, large-load circuit labeling, and control pathways; avoid over-investing in a small critical-loads subpanel unless the electrician/installer says it is uniquely cheap or required now.
- Keep HVAC, heat-pump water heater, dryer, oven/range, EV charger, pool/spa, and other large-load circuits identifiable and accessible for future smart load modules/contactors/smart-panel work.
- Add Ethernet/low-voltage/control conduit to the solar/gateway/battery equipment areas.
- Photograph and label open-wall routing, panel bays, blocking, and conduits before insulation/drywall.
- Keep plumbing/gas/storage obstructions away from likely equipment walls and keep roof penetrations away from the prime south-facing solar plane.
- Coordinate with electrician and, if possible, a future solar installer before drywall; final BESS location/design still needs code, fire, manufacturer, and permitting review.

## Known deployment quirk

Wrangler may warn that `/opt/data/hermes-pages/wrangler.jsonc` is missing `pages_build_output_dir` and is ignored. This warning is non-blocking for direct `wrangler pages deploy /opt/data/hermes-pages --project-name hermes-pages --commit-dirty=true` deployments; still verify live content afterward.
