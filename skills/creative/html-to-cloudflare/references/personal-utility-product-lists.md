...
- Lamps Plus often blocks direct Python fetch with `403`; `web_extract` may also fail. Use the Lamps Plus title/style code in `web_search` to identify manufacturer SKU and mirrors, then keep Gordon's original Lamps Plus URL as the card link. Good mirrors: Lighting New York brand subdomains (`accesslightinglights.com`, `lightingnewyork-modernforms.com`, `afx.lightingnewyork.com`) expose product pages and direct images; LBC Lighting Shopify pages expose `/products/<handle>.js`; Amazon can expose manufacturer images for AFX. Examples: Access Citi 18 `62570LEDD-BS/ACR`; ET2 Baritone 36 `E23406-01SN` via LBC Shopify; Modern Forms Loft 18 `WS-3618-BN` (not Mini Loft `WS-14818-BN`); AFX Axel 48 `AXLV4804LAJUDSN` via Lighting New York/Amazon. Use mirror metadata/images but preserve the original Lamps Plus URL.
...
## Flush mount lights command semantics

Primary current command:
- `add flush mount <URL>`

Canonical source and URL:
- Source: `/opt/data/hermes-pages/flush-mount-lights.html`
- URL: `https://hermes-pages-d55.pages.dev/flush-mount-lights`
- Display title/H1/wiki link text: **Flush Mount Lights for Entry Foyer** / **Flush Mount Lights**

This page is for flush-mount and semi-flush ceiling lights being considered for the Ventura entry foyer. For each new product, prioritize metadata that matters for an entry fixture:
- Overall diameter/height and whether scale works for the foyer ceiling height
- Finish and visual style from below
- Integrated LED vs replaceable bulbs, bulb base, wattage/lumen output, CCT, CRI if available
- Dimming compatibility
- Price and installation notes
- A concise entry-foyer fit note: welcoming light, glare, warmth, finish match, and first-impression look
...
- Vakkerlight product pages expose Shopify JSON at `/products/<handle>.js`; Shopify CDN images download cleanly with browser-like `User-Agent` + `Referer: https://vakkerlight.com/`, and clean isolated product images are often among the first few `images` entries. Use vision verification before saving.
  - For `wooden-geometric-ceiling-light`, variant `44376442798321` is `Circle: ∅ 19.7″ x H 2.3″ / Wood color / Warm White`, price `$148`; description includes natural rubberwood, acrylic diffuser, integrated LED, 3000K warm white standard, 4000K/6000K by request, and external driver required for dimmable version because of the slim 2.3" profile.
  - For `lotus-leaf-acrylic-ceiling-lamp`, price range is `$142–$840`; circular sizes are ∅11.8/15.7/20.5/28.3 in. × H1.6 in. and rectangular sizes go up to 53.1 × 33.5 × H2.4 in. Specs from the product description: metal + acrylic body, hydrographic natural/walnut wood-grain finish (not solid wood), white acrylic diffuser, integrated LED, 3000K/4000K/6000K variants, warm-light or Triac warm-dimming variants, AC 110–240V, hardwired flush mount, IP20 indoor use. A good verified local image filename: `assets/flush-vakker-lotus-leaf-acrylic-ceiling-lamp.jpg`.
